 ...
 * 44x44px touch targets, interactive Explore action, and WCAG AA high-contrast dark mode support.
 */
    ...
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
      ...
      >
        ...
        <div className="space-y-1.5">
          <h3 className="text-base font-extrabold text-nova-charcoal dark:text-slate-100 leading-snug line-clamp-1 group-hover:text-nova-coral transition-colors">
            {course.title}
          </h3>
        ...
        {/* Footer Row with Active Explore Button */}
        <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-slate-800">
          ...
          <button
            onClick={handleExploreClick}
            aria-label={`Explore course details for ${course.title}`}
            className="px-4 py-2.5 rounded-xl bg-nova-charcoal dark:bg-slate-100 text-white dark:text-nova-charcoal text-xs font-bold flex items-center gap-1.5 transition-all hover:scale-105 group-hover:bg-nova-coral group-hover:text-white min-h-[44px] cursor-pointer"
          >
            <span>Explore</span>
